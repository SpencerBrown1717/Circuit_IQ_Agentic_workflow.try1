...
class JobManager:
    ...
    def create_job(self) -> str:
        ...
        self.jobs[job_id] = JobStatus(
            ...
            status="created",
            completed_tasks=[],
            pending_tasks=self.queues.copy(),
                ...
                if specs:
                    ...
                    for queue in queue_mapping.get(category, []):
                        ...
                        try:
                            ...
                            logger.info(f"Requirements sent to {queue} for job {job_id}")
                        except Exception as e:
                            logger.error(f"Failed to publish message to {queue}: {e}")
                            self.jobs[job_id].errors.append(f"Failed to distribute task to {queue}")
...
async def submit_datasheet(
    ...
    gerber_files: Optional[List[UploadFile]] = None
):
    """Submit datasheet and optional Gerber files for processing"""
    try:
        ...
        # Process datasheet and distribute tasks (do this in background to avoid timeout)
        if background_tasks:
            background_tasks.add_task(app.state.job_manager.analyze_and_distribute, job_id, datasheet_content)
        else:
            app.state.job_manager.analyze_and_distribute(job_id, datasheet_content)
        
        # Process Gerber files if provided
        if gerber_files:
            files = {}
            for file in gerber_files:
                content = await file.read()
                # Extract name without extension and convert to queue name format
                name = file.filename.split('.')[0].lower().replace('-', '_')
                
                # Map filename to proper queue name if possible
                queue_name_mapping = {
                    "top": "copper_top",
                    "bottom": "copper_bottom",
                    "mask_top": "soldermask_top",
                    "mask_bottom": "soldermask_bottom",
                    "silk": "silkscreen",
                    "drill": "drill"
                }
                
                for key, queue in queue_name_mapping.items():
                    if key in name:
                        name = queue
                        break
                
                if name in app.state.job_manager.queues:
                    files[name] = content
                else:
                    logger.warning(f"Couldn't map file {file.filename} to a known queue")
        ...
        return JSONResponse(
            ...
            content={
                ...
                "requirements": app.state.job_manager.jobs[job_id].extracted_requirements
            }
        )
